...
- Router AST validator now rejects dunder/underscored string-constant subscripts (#050). `state['__class__']`, `state['_data']`, `state['_secret']` previously bypassed the attribute-only guard in `_validate_expression` and reached `DotAccessDict.__getitem__('_x')`, which delegated to `object.__getattribute__` and returned the wrapper's raw underlying dict — leaking every user-seeded `_secret` / `_token` / `_internal` key. The validator now applies the same `_reject_attribute` check to `ast.Subscript` slices when the slice is an `ast.Constant` of type `str`, and `DotAccessDict.__getattr__` no longer falls back to `object.__getattribute__` so the runtime defence holds even if a future grammar tweak misses the AST gate. Numeric subscripts (`state['items'][0]`) and ordinary keys (`state['user']`) are unaffected.
- `approval_gate.notify.url` now passes through the workflow sandbox before the POST (#051). `webhooks/sender.send_webhook` accepts an optional `sandbox: ToolSandbox | None` and consults `ToolSandbox.validate_webhook_url` ahead of every delivery attempt; the approval-gate step builds the sandbox from `StepContext.sandbox_config` so the gate's notification surface is gated by the same allowlist as every other network operation. When the workflow declares `config.sandbox.enabled: true`, the destination must satisfy `allow_network`, `allowed_schemes`, and `allowed_domains`. When the sandbox is **disabled**, a built-in deny-list still blocks loopback, link-local (including cloud metadata at `169.254.169.254`), RFC 1918, and any non-`http`/`https` scheme. Workflows that genuinely need to notify an in-cluster service can opt out per-workflow via `sandbox.allow_internal_webhook_targets: true` (also a new field on `SandboxConfig`). A blocked URL is logged and emitted to the observer as `on_webhook_delivery(step_id, workflow_name, "sandbox_blocked", 0.0)`; the workflow's pause itself is unaffected because the pause and the notification are independent.
- `ToolSandbox.validate_path` wraps `ValueError` / `OSError` (#051). Null-byte paths and other inputs that `Path.resolve()` refuses now surface as `SandboxViolationError` with the original path in the message — callers that catch only the sandbox exception class no longer miss the case.
- State-value redaction policy for persisted artefacts (#052). New `agentloom.core.redact` module ships `RedactionPolicy` (glob patterns, env-var merge), `redact_state(state, policy)`, and a stable `<REDACTED:sha256=...>` sentinel; a workflow author declares per-key redaction via `state_schema:` in YAML (`state_schema: {api_key: {redact: true}, "*token*": {redact: true}}`) or deployment-wide via `AGENTLOOM_REDACT_STATE_KEYS=api_key,password,*token*`, and the engine merges the two into a single policy at construction time. The policy applies at every persistence boundary: `WorkflowEngine._save_checkpoint` redacts both `state` and the literal `state:` block carried inside `workflow_definition` (so the YAML seed cannot reappear under a different key); the webhook sender redacts the rendered `body_template` so notification destinations never receive the plaintext; `llm_call`'s opt-in `capture_prompts` span event is re-rendered against the redacted state so the trace backend sees the sentinel even when the wire actually carried the secret. The in-memory state stays plaintext so a step that legitimately interpolates `{state.api_key}` against `api.openai.com` keeps working — only persisted copies are masked. The sentinel is hash-stable (sha256 of the value's string form, truncated to 16 hex chars) so two checkpoints carrying the same secret diff identically without ever exposing the plaintext. Resume contract is documented: a redacted checkpoint cannot be resumed with the original secret, so a workflow that pauses on `approval_gate` before consuming the secret should either re-inject it on resume (`--state api_key=...`) or omit the key from `state_schema`. Lists of secrets collapse to a list of sentinels (one per element) so consumers that read shape don't break; nested dicts redact element-wise.
